                    ...
                    groups: [
                        ...
                        // 2) Chunks por tipo de gráfico de Recharts (name dinámico)
                        {
                            ...
                            name: (id) => {
                                if (!/node_modules[\\/]recharts/.test(id))
                                    return null;
                                // line
                                if (/(LineChart|[\\/]cartesian[\\/]Line(\.|[\\/]))/.test(id))
                                    return 'rechart-line';
                                // bar
                                if (/(BarChart|[\\/]cartesian[\\/]Bar(\.|[\\/]))/.test(id))
                                    return 'rechart-bars';
                                // area
                                if (/(AreaChart|[\\/]cartesian[\\/]Area(\.|[\\/]))/.test(id))
                                    return 'rechart-area';
                                // pie / donut
                                if (/(PieChart|[\\/]polar[\\/]Pie(\.|[\\/]))/.test(id))
                                    return 'rechart-pie';
                                // radar
                                if (/(RadarChart|[\\/]polar[\\/]Radar(\.|[\\/]))/.test(id))
                                    return 'rechart-radar';
                                // radial bar
                                if (/(RadialBarChart|[\\/]polar[\\/]RadialBar(\.|[\\/]))/.test(id))
                                    return 'rechart-radialbar';
                                // scatter
                                if (/(ScatterChart|[\\/]cartesian[\\/]Scatter(\.|[\\/]))/.test(id))
                                    return 'rechart-scatter';
                                // treemap
                                if (/([\\/]Treemap(\.|[\\/]))/.test(id))
                                    return 'rechart-treemap';
                        ...
                        // 4) Chunks por servicio de Firebase (name dinámico)
                        {
                            name: (id) => {
                                ...
                                // analytics
                                if (/([\\/]analytics[\\/])/.test(id))
                                    return 'firebase-analytics';
                                // performance
                                if (/([\\/]performance[\\/])/.test(id))
                                    return 'firebase-performance';
                                // remote-config
                                if (/([\\/]remote-config[\\/])/.test(id))
                                    return 'firebase-remote-config';
                                // installations
                                if (/([\\/]installations[\\/])/.test(id))
                        ...
                        // 6) Chunks por componente de Radix UI (name dinámico)
                        {
                            name: (id) => {
                                ...
                                if (/(react-dialog)/.test(id))
                                    return 'radix-dialog';
                                // dropdown-menu
                                if (/(react-dropdown-menu)/.test(id))
                                    return 'radix-dropdown-menu';
                                // select
                                if (/(react-select)/.test(id))
                                    return 'radix-select';
                                // tooltip
                                if (/(react-tooltip)/.test(id))
                                    return 'radix-tooltip';
                                // popover
                                if (/(react-popover)/.test(id))
                                    return 'radix-popover';
                                // separator
                                if (/(react-separator)/.test(id))
                                    return 'radix-separator';
                                // accordion
                                if (/(react-accordion)/.test(id))
                                    return 'radix-accordion';
                                // alert-dialog
                                if (/(react-alert-dialog)/.test(id))
                                    return 'radix-alert-dialog';
                                // checkbox
                                if (/(react-checkbox)/.test(id))
                                    return 'radix-checkbox';
                                // collapsible
                                if (/(react-collapsible)/.test(id))
                                    return 'radix-collapsible';
                                // hover-card
                                if (/(react-hover-card)/.test(id))
                                    return 'radix-hover-card';
                                // label
                                if (/(react-label)/.test(id))
                                    return 'radix-label';
                                // menubar
                                if (/(react-menubar)/.test(id))
                                    return 'radix-menubar';
                                // navigation-menu
                                if (/(react-navigation-menu)/.test(id))
                                    return 'radix-navigation-menu';
                                // progress
                                if (/(react-progress)/.test(id))
                                    return 'radix-progress';
                                // radio-group
                                if (/(react-radio-group)/.test(id))
                                    return 'radix-radio-group';
                                // scroll-area
                                if (/(react-scroll-area)/.test(id))
                                    return 'radix-scroll-area';
                                // slider
                                if (/(react-slider)/.test(id))
                                    return 'radix-slider';
                                // switch
                                if (/(react-switch)/.test(id))
                                    return 'radix-switch';
                                // tabs
                                if (/(react-tabs)/.test(id))
                                    return 'radix-tabs';
                                // toggle
                                if (/(react-toggle)/.test(id))
                                    return 'radix-toggle';
                                // toggle-group
                                if (/(react-toggle-group)/.test(id))
                                    return 'radix-toggle-group';
                                // toolbar
                                if (/(react-toolbar)/.test(id))
                                    return 'radix-toolbar';
